...
export const useUpdateTaskMutation = (slug?: string) => {
  const { toast } = useToast();

  return useMutation({
    mutationFn: ({ taskId, data }: { taskId: string; data: any }) =>
      taskService.updateTask(slug!, taskId, data),
    onSuccess: () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, taskId) => {
      queryClient.setQueryData(
        projectId ? ["tasks", slug, projectId] : ["tasks", slug],
        (old: any) => {
...
export const useAddSubtaskMutation = (slug: string, taskId: string) => {
  ...
  return useMutation({
    mutationFn: (data: { title: string }) =>
      taskService.addSubtask(slug, taskId, data),
    onSuccess: (res) => {
      const newSubtask = res;
      queryClient.setQueryData(["task", taskId, slug], (old: any) => {
        if (!old) return old;
        if (old.subtasks?.some((st: any) => st.id === newSubtask.id))
          return old;
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error.response?.data?.message || "Access denied or server error.",
      });
    },
  });
};
...
export const useDeleteSubtaskMutation = (slug: string, taskId: string) => {
  ...
  return useMutation({
    mutationFn: (subtaskId: string) =>
      taskService.deleteSubtask(slug, taskId, subtaskId),
...
export const useAddCommentMutation = (slug: string, taskId: string) => {
  ...
  return useMutation({
    mutationFn: (data: { content: string }) =>
      taskService.addComment(slug, taskId, data),
    onSuccess: (res) => {
  ...
  return useMutation({
    ...
    onSuccess: (res) => {
      ...
      queryClient.setQueryData(["task", taskId, slug], (old: any) => {
        ...
        if (data?.watching) {
          if (!newWatchers.some((w: any) => w.userId === currentUser.id)) {
            newWatchers = [
              ...newWatchers,
              { userId: currentUser.id, user: { name: currentUser.name } },
            ];
          }
        } else {
          newWatchers = newWatchers.filter(
      ...
      toast({
        ...
        description: data?.watching
          ? "You will now receive alerts for this task."
          : "You have unwatched this task.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        title: "Upload Failed",
        description:
          error.response?.data?.message || "File must be under 10MB.",
      });
    },
  });
};
...
export const useDeleteAttachmentMutation = (slug: string, taskId: string) => {
  ...
  return useMutation({
    mutationFn: (attachmentId: string) =>
      taskService.deleteAttachment(slug, attachmentId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["task", taskId, slug] });
      queryClient.invalidateQueries({ queryKey: ["tasks", slug] });
